module.exports = {
    createPlayground: (req, res, next) => {
        ...
        const { name, address, city, state, zip,img, reviews } = req.body;
        // playground = { id: playgrounds[playgrounds.length - 1].d + 1 }

        // playgrounds.push(playgrounds);
        
        dbInstance.create_playground([name, address, city, state, zip,img, reviews])
    ...
    getPlayground: (req, res, next) => {
        ...
        dbInstance.read_playground([ params.id ])
        .then( playground => res.status(200).send(playground))
        .catch( () => res.status(500).send());
    },
    getPlaygrounds: (res, req, next ) => {
        const dbInstance = req.app.get('db');
